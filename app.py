...
def load_css():
    ...
    .has-corners::before { top: 20px; left: 20px; border-width: 2px 0 0 2px; }
    .has-corners::after { bottom: 20px; right: 20px; border-width: 0 2px 2px 0; }
    ...
    .stTextInput input, .stNumberInput input, .stTextArea textarea, .stMultiselect div[data-baseweb="select"] > div { background-color: var(--widget-bg) !important; color: var(--text-primary) !important; border: 1px solid var(--border-color) !important; border-radius: var(--border-radius-md) !important; transition: all 0.3s ease; }
...
def create_header(main_logo, partner_logos):
    ...
    partner_html = "".join(f'<img src="data:image/png;base64,{b64}" alt="{name} Logo" title="{name}">' for name, b64 in partner_logos_b64.items() if b64)

    if main_logo_b64:
        st.markdown(f"""
        <div class="logo-container">
            <div class="main-logo"><img src="data:image/png;base64,{main_logo_b64}" alt="AllWave AV Logo"></div>
            <div class="partner-logos">{partner_html}</div>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.warning("Main company logo not found. Please check the 'assets' folder.")
...
def show_login_page(logo_b64, page_icon_path):
    ...
    logo_html = f'<img src="data:image/png;base64,{logo_b64}" class="login-main-logo" alt="AllWave AV Logo">' if logo_b64 else ''
...
def main():
    ...
    partner_logos_paths = { "Crestron": Path("assets/crestron_logo.png"), "AVIXA": Path("assets/avixa_logo.png"), "ISO Certified": Path("assets/iso_logo.png") }
    ...
    with st.sidebar:
        st.markdown(f'''<div class="user-info"><h3>👤 Welcome</h3><p>{st.session_state.get("user_email", "Unknown")}</p></div>''', unsafe_allow_html=True)
        if st.button("🚪 Logout", use_container_width=True):
            show_animated_loader("De-authorizing...", 1); st.session_state.clear(); st.rerun()
        
        st.markdown("<hr style='border-color: var(--border-color);'>", unsafe_allow_html=True)
        
        st.markdown('<div class="sidebar-section"><h3>🚀 Mission Parameters</h3></div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="sidebar-section"><h3>✨ Key Feature Requirements</h3></div>', unsafe_allow_html=True)
        st.session_state.feature_tags = st.multiselect(
            "Select required product features:",
            ['wireless_presentation', 'interactive', '4k', 'zoom_certified', 'teams_certified'],
            key="feature_multiselect"
        )
        # --------------------------------------------------
        
        st.markdown('<div class="sidebar-section"><h3>🌐 Environment Design</h3></div>', unsafe_allow_html=True)
    ...
    # --- Main Content Tabs ---
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["📝 Project Scope", "📐 Room Analysis", "📋 Requirements", "🛠️ Generate BOQ", "✨ 3D Visualization"])

    with tab3: # Requirements Tab
         ...
         technical_reqs = {}
         st.text_area("🎯 Specific Client Needs & Features:", key="features_text_area", placeholder="e.g., 'Must be Zoom certified, requires wireless presentation...'", height=100)
         technical_reqs.update(create_advanced_requirements()) # Assuming this returns a dict
         st.markdown('</div>', unsafe_allow_html=True)

    with tab4: # Generate BOQ Tab
        st.markdown('<div class="glass-container interactive-card has-corners">', unsafe_allow_html=True)
            ...
            if not model:
                ...
                try:
                    ...
                    boq_items, avixa_calcs, equipment_reqs = generate_boq_from_ai(
                        ...
                        st.session_state.get('feature_tags', []) # Pass the selected tags
                    )
                    ...
                    if boq_items:
                        ...
                        processed_boq, engineering_warnings = _perform_engineering_validation(processed_boq, product_df)
                        
                        st.session_state.boq_items = processed_boq
                        update_boq_content_with_current_items()
                        
                        all_warnings = avixa_validation.get('avixa_warnings', []) + engineering_warnings
                        st.session_state.validation_results = {"warnings": all_warnings}
                    ...
                    progress_bar.empty(); show_error_message(f"An error occurred: {e}")
