...
def calculate_avixa_recommendations(room_length, room_width, room_height, room_type):
    ...
    total_av_power = display_power + audio_system_power + 25 + 100 + 75 # camera, network, control
    circuit_requirement = "15A Standard Circuit" if total_av_power < 1200 else "20A Dedicated Circuit" if total_av_power < 1800 else "Multiple 20A Circuits"
    ups_runtime_minutes = 30 if 'executive' in room_type.lower() else 15 if 'training' in room_type.lower() else 10
    ...
    return {
        ...
        'recommended_bandwidth_mbps': int((2.5 * estimated_people) + 5.0 + 10),
        ...
        'cable_runs': {
            'cat6a_network': 3 + (estimated_people // 6),
            'hdmi_video': 2,
            'xlr_audio': estimated_people // 3,
            'power_circuits': 2 + (total_av_power // 1000)
        },
...
def determine_equipment_requirements(avixa_calcs, room_type, technical_reqs):
    ...
    reqs = {
        'displays': {
            'type': "Commercial LED Display" if display_size <= 55 else "Large Format Display" if display_size <= 75 else "Professional Large Format Display" if display_size <= 86 else "Video Wall or Laser Projector",
            'size_inches': display_size, 'quantity': avixa_calcs['recommended_display_count'], 'resolution': '4K' if display_size > 43 else '1080p', 'mounting': 'Wall Mount' if display_size < 75 else 'Heavy Duty Wall Mount'
        },
        'audio_system': {},
        'video_system': {
            'camera_type': 'Fixed Wide-Angle Camera' if avixa_calcs['estimated_occupancy'] <= 6 else 'PTZ Camera with Auto-Framing' if avixa_calcs['estimated_occupancy'] <= 12 else 'Multi-Camera System with Tracking',
            'camera_count': 1 if avixa_calcs['estimated_occupancy'] <= 12 else 2, '4k_required': avixa_calcs['estimated_occupancy'] > 8
        },
        'control_system': {
            'type': 'Native Room System Control' if room_volume < 2000 else 'Touch Panel Control' if room_volume < 5000 else 'Advanced Programmable Control System',
            'touch_panel_size': '7-inch' if room_volume < 3000 else '10-inch', 'integration_required': room_volume > 5000
        },
        'infrastructure': {
            'equipment_rack': 'Wall-Mount' if room_volume < 3000 else 'Floor-Standing', 'rack_size': '6U' if room_volume < 3000 else '12U' if room_volume < 8000 else '24U',
            'cooling_required': avixa_calcs['total_power_load_watts'] > 1500, 'ups_required': True
        },
        'compliance': []
    }
...
def validate_avixa_compliance(boq_items, avixa_calcs, equipment_reqs):
    ...
    if not displays: issues.append("CRITICAL: No display found in BOQ")
    else:
        for display in displays:
            size_match = re.search(r'(\d+)"', display.get('name', ''))
            if size_match and abs(int(size_match.group(1)) - avixa_calcs['detailed_viewing_display_size']) > 10:
                warnings.append(f"Display size deviates from AVIXA calculation.")
    if equipment_reqs['audio_system'].get('dsp_required') and not any('dsp' in i.get('name', '').lower() for i in boq_items): issues.append("CRITICAL: DSP required but not found in BOQ")
    if avixa_calcs['ups_va_required'] > 1000 and not any('ups' in i.get('name', '').lower() for i in boq_items): issues.append("CRITICAL: UPS system required but not found in BOQ")
    if avixa_calcs['requires_ada_compliance'] and not any(term in i.get('name', '').lower() for term in ['assistive', 'ada', 'loop'] for i in boq_items): issues.append("CRITICAL: ADA compliance required but no assistive devices in BOQ")
...
def estimate_power_draw(category, name):
    ...
    if 'display' in category.lower():
        if any(s in name_lower for s in ['55', '60', '65']): return 200
        if any(s in name_lower for s in ['70', '75', '80']): return 300
        if any(s in name_lower for s in ['85', '90', '98']): return 400
        return 150
    if 'audio' in category.lower():
        if 'amplifier' in name_lower: return 300
        if 'speaker' in name_lower: return 60
        if 'microphone' in name_lower: return 15
        return 50
    if 'video conferencing' in category.lower():
        if 'rally' in name_lower or 'bar' in name_lower: return 90
        if 'camera' in name_lower: return 25
        return 40
    if 'control' in category.lower(): return 75
    return 25
    ...
    for line in boq_content.split('\n'):
        ...
        if in_table and line.startswith('|') and 'TOTAL' not in line.upper():
            ...
            if len(parts) >= 6:
                ...
                matched = match_product_in_database(parts[2], parts[1], product_df)
                item_data = {
                    'name': parts[2], 'brand': parts[1], 'category': parts[0], 'quantity': quantity, 'price': price,
                    'justification': parts[6] if len(parts) > 6 else "Essential AV component.", 'specifications': parts[3],
                    'image_url': '', 'gst_rate': 18, 'matched': False
                }
                if matched:
                    item_data.update({
                        'name': matched.get('name', parts[2]), 'brand': matched.get('brand', parts[1]), 'category': matched.get('category', parts[0]),
                        'price': float(matched.get('price', price)), 'image_url': matched.get('image_url', ''), 'gst_rate': matched.get('gst_rate', 18), 'matched': True
                    })
                else: item_data['category'] = normalize_category(parts[0], parts[2])
                item_data['power_draw'] = estimate_power_draw(item_data['category'], item_data['name'])
...
def create_multi_room_interface():
    ...
    if st.session_state.project_rooms:
        ...
        if st.button(f"🗑️ Remove '{selected_room['name']}'", type="secondary"):
            ...
            else: st.session_state.boq_items, st.session_state.boq_content, st.session_state.validation_results, st.session_state.avixa_calcs = [], None, None, None
            st.rerun()
...
def create_room_calculator():
    ...
    recommended = next((t for t, s in ROOM_SPECS.items() if s["area_sqft"][0] <= area <= s["area_sqft"][1]), None)
    if recommended: c2.success(f"Recommended Type: {recommended}")
    else: c2.warning("Room size is outside typical ranges")
    return area, height
...
def create_advanced_requirements():
    ...
    with c1:
        ...
        has_circuit = st.checkbox("Dedicated 20A Circuit Available", key="dedicated_circuit_checkbox")
        network = st.selectbox("Network Infrastructure", ["Standard 1Gb", "10Gb Capable", "Fiber Available"], key="network_capability_select")
        cable = st.selectbox("Cable Management", ["Exposed", "Conduit", "Raised Floor", "Drop Ceiling"], key="cable_management_select")
    with c2:
        st.write("**Compliance & Standards**")
        ada = st.checkbox("ADA Compliance Required", key="ada_compliance_checkbox")
        fire = st.checkbox("Fire Code Compliance Required", key="fire_code_compliance_checkbox")
        security = st.selectbox("Security Level", ["Standard", "Restricted", "Classified"], key="security_clearance_select")
...
def edit_current_boq(currency):
    ...
    for i, item in enumerate(st.session_state.boq_items):
        with st.expander(f"{item.get('category', 'N/A')} - {item.get('name', '')[:50]}"):
            c1, c2, c3, c4 = st.columns([2, 2, 1, 1])
            new_name = c1.text_input("Product Name", value=item.get('name', ''), key=f"name_{i}")
            new_brand = c1.text_input("Brand", value=item.get('brand', ''), key=f"brand_{i}")
            cat_list = ['Displays', 'Audio', 'Video Conferencing', 'Control', 'Mounts', 'Cables', 'General', 'Infrastructure', 'Services']
            curr_cat = item.get('category', 'General')
            new_cat = c2.selectbox("Category", cat_list, index=cat_list.index(curr_cat) if curr_cat in cat_list else 6, key=f"cat_{i}")
            new_qty = c3.number_input("Quantity", min_value=1, value=int(item.get('quantity', 1)), key=f"qty_{i}")
            curr_price = float(item.get('price', 0))
            disp_price = convert_currency(curr_price, 'INR') if currency == 'INR' else curr_price
            new_price = c3.number_input(f"Unit Price ({currency})", min_value=0.0, value=disp_price, format="%f", key=f"price_{i}")
            stored_price = new_price / get_usd_to_inr_rate() if currency == 'INR' else new_price
            if c4.button("Remove", key=f"remove_{i}", type="secondary"): items_to_remove.append(i)
            st.session_state.boq_items[i].update({'name': new_name, 'brand': new_brand, 'category': new_cat, 'quantity': new_qty, 'price': stored_price})
    if items_to_remove:
...
def add_products_interface(product_df, currency):
    ...
    c1, c2 = st.columns([2, 1])
    cats = ['All'] + sorted(list(product_df['category'].unique()))
    sel_cat = c1.selectbox("Filter by Category", cats, key="add_cat_filter")
    filtered_df = product_df[product_df['category'] == sel_cat] if sel_cat != 'All' else product_df
    prod_opts = [f"{row['brand']} - {row['name']}" for _, row in filtered_df.iterrows()]
    if not prod_opts:
        st.warning("No products found for this category.")
        return
    sel_prod_str = c1.selectbox("Select Product", prod_opts, key="add_prod_select")
    sel_prod = next((row for _, row in filtered_df.iterrows() if f"{row['brand']} - {row['name']}" == sel_prod_str), None)
    if sel_prod is not None:
        qty = c2.number_input("Quantity", 1, key="add_prod_qty")
        if c2.button("Add to BOQ", type="primary"):
            new_item = {'category': sel_prod.get('category', 'General'), 'name': sel_prod.get('name', ''), 'brand': sel_prod.get('brand', ''), 'quantity': qty, 'price': float(sel_prod.get('price', 0)), 'justification': 'Manually added component.', 'specifications': sel_prod.get('features', ''), 'image_url': sel_prod.get('image_url', ''), 'gst_rate': sel_prod.get('gst_rate', 18), 'matched': True}
            st.session_state.boq_items.append(new_item)
            update_boq_content_with_current_items()
            st.success(f"Added {qty}x {sel_prod['name']}!")
            st.rerun()

def product_search_interface(product_df, currency):
    st.write("**Search Full Product Catalog:**")
    search_term = st.text_input("Search products...", placeholder="Enter name, brand, or features", key="search_term_input")
    if search_term:
        mask = product_df[['name', 'brand', 'features']].apply(lambda x: x.astype(str).str.contains(search_term, case=False, na=False)).any(axis=1)
        results = product_df[mask]
        st.write(f"Found {len(results)} products:")
        for i, prod in results.head(10).iterrows():
            with st.expander(f"{prod.get('brand', '')} - {prod.get('name', '')[:60]}"):
                c1, c2, c3 = st.columns([2, 1, 1])
                c1.write(f"**Category:** {prod.get('category', 'N/A')}")
                if pd.notna(prod.get('features')): c1.write(f"**Features:** {str(prod['features'])[:100]}...")
                qty = c3.number_input("Qty", 1, key=f"search_qty_{i}")
                if c3.button("Add", key=f"search_add_{i}"):
                    new_item = {'category': prod.get('category', 'General'), 'name': prod.get('name', ''), 'brand': prod.get('brand', ''), 'quantity': qty, 'price': float(prod.get('price', 0)), 'justification': 'Added via search.', 'specifications': prod.get('features', ''), 'image_url': prod.get('image_url', ''), 'gst_rate': prod.get('gst_rate', 18), 'matched': True}
                    st.session_state.boq_items.append(new_item)
                    update_boq_content_with_current_items()
...
def get_sample_product_data():
    return [{'name': 'Samsung 55" QM55R 4K Display', 'brand': 'Samsung', 'category': 'Displays', 'price': 1200, 'features': '55" 4K UHD, 500-nit, 16/7', 'image_url': 'https://image-us.samsung.com/SamsungUS/samsung/us/business/displays/digital-signage/qm-r-series/qm55r/gallery/QM55R_001_Front_Black_1600x1200.jpg', 'gst_rate': 18}, {'name': 'Logitech Rally Bar', 'brand': 'Logitech', 'category': 'Video Conferencing', 'price': 2700, 'features': 'All-in-one 4K video bar', 'image_url': 'https://resource.logitech.com/w_692,c_lpad,ar_1:1,q_auto,f_auto,dpr_1.0/d_transparent.gif/content/dam/logitech/en/products/video-conferencing/rally-bar/gallery/rally-bar-gallery-1-graphite.png', 'gst_rate': 18}, {'name': 'Shure MXA920 Ceiling Array', 'brand': 'Shure', 'category': 'Audio', 'price': 1800, 'features': 'Ceiling mic array, Dante', 'image_url': 'https://www.shure.com/images/pr_mxa920_large.png', 'gst_rate': 18}, {'name': 'QSC CP8T Ceiling Speaker', 'brand': 'QSC', 'category': 'Audio', 'price': 280, 'features': '8" ceiling speaker, 70V/100V', 'image_url': 'https://www.qsc.com/resource/blob/24070/52e8f8d8b8d8c4a1c3e6e8d8e8d8e8d8/cp8t-data.jpg', 'gst_rate': 18}, {'name': 'APC SMT1500 UPS', 'brand': 'APC', 'category': 'Infrastructure', 'price': 450, 'features': '1500VA UPS', 'image_url': 'https://www.apc.com/resource/include/techspec_index/images/smt1500.jpg', 'gst_rate': 18}, {'name': 'Cat6A Cable (per 100ft)', 'brand': 'Belden', 'category': 'Cables', 'price': 85, 'features': 'Cat6A network cable, Plenum', 'image_url': '', 'gst_rate': 18}]
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("Login", type="primary", use_container_width=True):
                if email.endswith(("@allwaveav.com", "@allwavegs.com")) and len(password) > 3:
                    st.session_state.authenticated, st.session_state.user_email = True, email; st.rerun()
...
def main():
    ...
    with st.sidebar:
        ...
        st.session_state['currency'] = st.selectbox("Currency Display", ["INR", "USD"], 0, key="currency_select")
        st.session_state.gst_rates['Electronics'] = st.number_input("Hardware GST (%)", 0, 28, 18, key="electronics_gst")
        st.session_state.gst_rates['Services'] = st.number_input("Services GST (%)", 0, 28, 18, key="services_gst")
    ...
    with tab3:
        st.subheader("Room & Project Requirements")
        c1, c2 = st.columns(2)
        with c1: room_area, ceiling_height = create_room_calculator()
        with c2: features = st.text_area("Specific Customer Requirements:", placeholder="e.g., 'Dual displays, wireless presentation, Zoom certified'", height=150, key="features_text_area")
        st.markdown("---")
    ...
    with tab4:
        ...
        if st.session_state.project_rooms:
            ...
            if excel_data:
                ...
                c2.download_button("📊 Download Full Project BOQ", excel_data, filename, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
