...
                    case 'production_studio': createStudioLayout(furnitureGroup, tableMaterial, chairMaterial, safeFurnitureSpec); break;
                    case 'telepresence': createTelepresenceLayout(furnitureGroup, tableMaterial, chairMaterial, safeFurnitureSpec); break;
                    default: createConferenceLayout(furnitureGroup, tableMaterial, chairMaterial, safeFurnitureSpec);
                }}
                scene.add(furnitureGroup);
            }}
...
                
                const chairRadius = tableRadius + toUnits(1.5);
                const maxChairs = Math.min(spec.chair_count, Math.floor((chairRadius * 2 * Math.PI) / toUnits(2)));
                for (let i = 0; i < maxChairs; i++) {{
                    const chair = createChair(chairMaterial);
                    const angle = (i / maxChairs) * Math.PI * 1.5 - Math.PI * 0.25; // Arrange in a semi-circle facing front
                    chair.position.x = Math.cos(angle) * chairRadius;
                    chair.position.z = Math.sin(angle) * chairRadius;
                    chair.rotation.y = -angle - Math.PI / 2;
                    
                    if (Math.abs(chair.position.x) < toUnits(roomDims.length/2 - 1.5) && 
                        Math.abs(chair.position.z) < toUnits(roomDims.width/2 - 1.5)) {{
                        group.add(chair);
                    }}
                }}
            }}
...
                const tableWidth = toUnits(spec.table_size[1] * 0.9);
                
                const shape = new THREE.Shape();
                const radius = tableWidth / 2;
                shape.moveTo(-tableLength/2, -radius);
                shape.lineTo(tableLength/2, -radius);
                shape.arc(0, 0, radius, -Math.PI/2, Math.PI/2, false);
                shape.lineTo(-tableLength/2, radius);
                shape.arc(0, 0, radius, Math.PI/2, -Math.PI/2, false);

                const extrudeSettings = {{ depth: toUnits(0.2), bevelEnabled: false }};
                const table = new THREE.Mesh(new THREE.ExtrudeGeometry(shape, extrudeSettings), tableMaterial);
                table.rotation.x = Math.PI / 2;
                table.position.y = toUnits(2.5);
                group.add(table);
                
                const chairSpacing = toUnits(2.8);
                const chairsPerSide = Math.min(Math.floor(spec.chair_count / 2), Math.floor(tableLength / chairSpacing));

                for(let i = 0; i < chairsPerSide; i++) {{
                    const xPos = -tableLength/2 + chairSpacing/2 + i * chairSpacing;
                    // Side 1
                    const chair1 = createChair(chairMaterial);
                    chair1.position.set(xPos, 0, -tableWidth/2 - toUnits(1.5));
                    group.add(chair1);
                    // Side 2
                    const chair2 = createChair(chairMaterial);
                    chair2.position.set(xPos, 0, tableWidth/2 + toUnits(1.5));
                    chair2.rotation.y = Math.PI;
                    group.add(chair2);
                }}
            }}
...
                const actualChairCount = Math.min(spec.chair_count, chairsPerSide * 2 + 2);
                
                for (let i = 0; i < Math.min(chairsPerSide, Math.floor((actualChairCount - 2)/2)); i++) {{
                    const xPos = -tableLength/2 + chairSpacing + i * chairSpacing;
                    
                    const chair1 = createChair(chairMaterial);
                    const chair1Z = -tableWidth/2 - toUnits(1.5);
                    if (Math.abs(chair1Z) < toUnits(roomDims.width/2 - 1.5)) {{
                        chair1.position.set(xPos, 0, chair1Z);
                        group.add(chair1);
                    }}
                    
                    const chair2 = createChair(chairMaterial);
                    const chair2Z = tableWidth/2 + toUnits(1.5);
                    if (Math.abs(chair2Z) < toUnits(roomDims.width/2 - 1.5)) {{
                        chair2.position.set(xPos, 0, chair2Z);
                        chair2.rotation.y = Math.PI;
                        group.add(chair2);
                    }}
...
                
                if (actualChairCount > chairsPerSide * 2) {{
                    const headChairX = -tableLength/2 - toUnits(2);
                    if (Math.abs(headChairX) < toUnits(roomDims.length/2 - 1.5)) {{
                        const headChair = createChair(chairMaterial);
                        headChair.position.set(headChairX, 0, 0);
                        headChair.rotation.y = Math.PI / 2;
                        group.add(headChair);
...
                }}
                if (actualChairCount > chairsPerSide * 2 + 1) {{
                    const footChairX = tableLength/2 + toUnits(2);
                    if (Math.abs(footChairX) < toUnits(roomDims.length/2 - 1.5)) {{
                        const footChair = createChair(chairMaterial);
                        footChair.position.set(footChairX, 0, 0);
                        footChair.rotation.y = -Math.PI / 2;
                        group.add(footChair);
...
            function createBoardroomLayout(group, tableMaterial, chairMaterial, spec) {{
                createConferenceLayout(group, tableMaterial, chairMaterial, spec); // Uses the same logic but could use createExecutiveChair
            }}
...
                    tableMaterial
                );
                instructorTable.position.set(0, toUnits(2.5), toUnits(-roomDims.width/2 + spec.table_size[1]/2 + 2));
                instructorTable.castShadow = true; instructorTable.receiveShadow = true;
                group.add(instructorTable);
                
                const whiteboard = new THREE.Mesh(new THREE.PlaneGeometry(toUnits(12), toUnits(4)), whiteboardMaterial);
...
            function createClassroomSeating(group, chairMaterial, chairCount) {{
                const rows = Math.min(5, Math.ceil(Math.sqrt(chairCount * (roomDims.width/roomDims.length))));
                const seatsPerRow = Math.ceil(chairCount / rows);
                const rowSpacing = toUnits(4);
                const seatSpacing = Math.min(toUnits(3.5), toUnits((roomDims.length - 2) / seatsPerRow));
                const startZ = toUnits(-roomDims.width/2 + 8);
                
                for (let row = 0; row < rows; row++) {{
                    for (let seat = 0; seat < seatsPerRow && (row * seatsPerRow + seat) < chairCount; seat++) {{
                        const chair = createChair(chairMaterial);
                        const chairX = -(seatsPerRow - 1) * seatSpacing / 2 + seat * seatSpacing;
                        const chairZ = startZ + row * rowSpacing;
                        
                        if (Math.abs(chairX) < toUnits(roomDims.length/2 - 1.5) && 
                            chairZ < toUnits(roomDims.width/2 - 1.5)) {{
                            chair.position.set(chairX, 0, chairZ);
                            group.add(chair);
                        }}
                    }}
...
            function createTheaterSeating(group, chairMaterial, chairCount) {{
                createClassroomSeating(group, chairMaterial, chairCount); // Using same layout for simplicity
            }}

            function createEventLayout(group, tableMaterial, chairMaterial, spec) {{
                const stageWidth = Math.min(toUnits(16), toUnits(roomDims.length * 0.7));
                const stageDepth = Math.min(toUnits(8), toUnits(roomDims.width * 0.2));
                const stage = new THREE.Mesh(new THREE.BoxGeometry(stageWidth, toUnits(0.5), stageDepth), tableMaterial);
                stage.position.set(0, toUnits(0.25), toUnits(-roomDims.width/2) + stageDepth/2 + toUnits(1));
                group.add(stage);

                const podsPerRow = Math.max(1, Math.floor((roomDims.length - 4) / 10));
                const podRows = Math.max(1, Math.floor((roomDims.width - stageDepth - 6) / 10));
                const totalPods = Math.min(podsPerRow * podRows, Math.floor(spec.chair_count / 6));
                
                for (let i = 0; i < totalPods; i++) {{
                    const row = Math.floor(i / podsPerRow);
                    const col = i % podsPerRow;
                    const podX = -(podsPerRow - 1) * toUnits(5) + col * toUnits(10);
                    const podZ = toUnits(-roomDims.width/2 + stageDepth + 6) + row * toUnits(10);
                    
                    const podTable = new THREE.Mesh(new THREE.CylinderGeometry(toUnits(3), toUnits(3), toUnits(0.2), 32), tableMaterial);
                    podTable.position.set(podX, toUnits(2.5), podZ);
                    group.add(podTable);
                    
                    for(let j = 0; j < 6; j++) {{
...
                        chair.position.set(podX + Math.cos(angle) * toUnits(4.5), 0, podZ + Math.sin(angle) * toUnits(4.5));
                        chair.rotation.y = -angle - Math.PI/2;
                        group.add(chair);
                    }}
                }}
...
            function createStudioLayout(group, tableMaterial, chairMaterial, spec) {{
                const consoleWidth = toUnits(spec.table_size[0]);
                const consoleDepth = toUnits(spec.table_size[1]);
                const console = new THREE.Mesh(new THREE.BoxGeometry(consoleWidth, toUnits(0.3), consoleDepth), tableMaterial);
                console.position.set(0, toUnits(2.5), toUnits(roomDims.width/2 - consoleDepth/2 - 2));
                group.add(console);
                
                const chairSpacing = toUnits(3);
                const maxChairs = Math.min(spec.chair_count, Math.floor(consoleWidth / chairSpacing));
                for (let i = 0; i < maxChairs; i++) {{
                    const chair = createExecutiveChair(chairMaterial);
                    const chairX = -(maxChairs - 1) * chairSpacing / 2 + i * chairSpacing;
                    const chairZ = toUnits(roomDims.width/2 - consoleDepth - 2);
                    chair.position.set(chairX, 0, chairZ);
                    chair.rotation.y = Math.PI;
                    group.add(chair);
                }}
...
                shape.lineTo(tableLength/2, tableWidth/2);
                shape.quadraticCurveTo(0, tableWidth/2 - toUnits(1), -tableLength/2, tableWidth/2);
                shape.lineTo(-tableLength/2, -tableWidth/2);
                
                const extrudeSettings = {{ depth: toUnits(0.2), bevelEnabled: false }};
                const table = new THREE.Mesh(new THREE.ExtrudeGeometry(shape, extrudeSettings), tableMaterial);
...
                group.add(table);
                
                const chairCount = Math.min(spec.chair_count, Math.floor(tableLength / toUnits(3)));
                const chairSpacing = tableLength / chairCount;
                
...
                    const chairZ = tableWidth/2 + toUnits(1.5);
                    
                    if (Math.abs(chairZ) < toUnits(roomDims.width/2 - 1.5)) {{
                        chair.position.set(chairX, 0, chairZ);
                        chair.rotation.y = Math.PI;
                        group.add(chair);
                    }}
...
                    y_ft = 2.6;
                    z_ft = 0;
                }} else if (type === 'network_switch' || type === 'control' || type === 'power') {{
                    x_ft = -roomDims.length / 2 + 1;
                    y_ft = 5 + (instanceIndex * (size[1] + 0.1));
                    z_ft = -roomDims.width / 2 + 1;
                 rotation = Math.PI / 2;
