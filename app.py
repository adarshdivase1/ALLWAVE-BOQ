...
class AIOptimizationEngine:
    ...
    def _generate_configuration(self, room_specs: RoomSpecification, 
        ...
        if tier == "economy":
            ...
            selected_speaker = max(speakers, key=lambda x: x.specifications.get("max_spl", 0))
            ...
            with st.expander(f"{product.brand} {product.name} - ${product.price_usd:,.2f}"):
                ...
                if st.button(f"Add {product.name}", key=f"add_{product.id}"):
                    if product not in st.session_state.selected_products:
                        ...
                        st.warning("Product already in BOQ")
...
def create_compliance_interface():
    ...
    with col3:
        critical_issues = len([r for r in results if r.critical_failures])
