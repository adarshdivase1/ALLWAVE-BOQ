...
def _populate_company_boq_sheet(sheet, items, room_details, styles, gst_rates):
    """Populate a single Excel sheet with BOQ data."""
    ...
    sheet['C5'], sheet['E5'] = "Room Name / Room Type", room_name
    sheet['C6'], sheet['C7'], sheet['C8'] = "Floor", "Number of Seats", "Number of Rooms"
    ...
    for row in sheet.iter_rows(min_row=header_start_row, max_row=sheet.max_row, min_col=1, max_col=len(headers1)):
        for cell in row:
            cell.font, cell.fill, cell.alignment = styles["table_header"], styles["table_header_fill"], Alignment(horizontal='center', vertical='center', wrap_text=True)
    ...
    for i, (category, cat_items) in enumerate(grouped_items.items()):
        ...
        for col_letter in 'ABCDEFGHIJKLMNOP': sheet[f'{col_letter}{cat_row_idx}'].fill = styles['group_header_fill']
        ...
        for item in cat_items:
            ...
            total_tax, total_with_gst = sgst_amount + cgst_amount, subtotal + (sgst_amount + cgst_amount)
            total_before_gst_hardware += subtotal; total_gst_hardware += total_tax
    ...
    for name, percentage in services:
        ...
        total_before_gst_services += amount_inr; total_gst_services += total_tax
    ...
    for cell in sheet[sheet.max_row]: cell.font, cell.fill = styles['bold'], styles['total_fill']
    ...
    for cell_id in [f'M{grand_total_row_idx}', f'N{grand_total_row_idx}']:
        sheet[cell_id].font, sheet[cell_id].fill, sheet[cell_id].alignment = styles["grand_total_font"], styles["grand_total_fill"], Alignment(horizontal='center')
...
def add_scope_of_work_sheet(workbook):
    ...
    for i, item in enumerate(scope_items, 3): sheet[f'A{i}'] = f"{i-2}. {item}"
